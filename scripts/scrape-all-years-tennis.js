...
async function waitCF(page, url) {
  ...
  for (let i = 0; i < 15; i++) { const t = await page.title(); if (!t.includes('instant') && !t.includes('moment')) break; await sleep(2000) }
}
...
async function main() {
  ...
  const openBrowser = async () => {
    ...
    await sleep(3000)
    browser = await puppeteerExtra.launch({ executablePath: findChrome(), headless: false, defaultViewport: null, userDataDir: path.join(process.env.LOCALAPPDATA || 'C:\\Users\\killi\\AppData\\Local', 'scrape-profile-tcdb'), args: ['--no-sandbox','--window-size=1280,900','--disable-blink-features=AutomationControlled'] })

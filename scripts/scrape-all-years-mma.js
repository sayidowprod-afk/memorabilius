...
// Segment "2023-Donruss-Optic-UFC" depuis un href ViewSet — réutilisé pour construire
// les URLs Checklist.cfm/Inserts.cfm (TCDB tolère aussi l'URL sans ce segment).
...
// /Inserts.cfm/sid/{sid} avec un lien vers leur propre Checklist.cfm.
async function fetchInsertSets(page, sid) {
  const ok = await navigate(page, `${TCDB}/Inserts.cfm/sid/${sid}`)
  await sleep(rand(300, 700))
  if (!ok) return []
  // Le libellé de section varie selon les sets ("Inserts", "Insert Sets (55)",
  // "Parallel Sets (26)", ...) — on prend directement tous les liens Checklist.cfm
  // de la page plutôt que de dépendre d'un intitulé de H3 précis. Le seul lien
  // Checklist.cfm parasite est celui du menu "Set Links" qui pointe vers le sid
  // du set parent lui-même (libellé "Checklist") — on l'exclut explicitement.

#!/usr/bin/env node
/**
 * Scraper TCDB Magic: The Gathering — Toutes années 2025→1993, Major Releases uniquement
 * Pas d'équipes : scraping direct depuis la page du set (ViewSet.cfm)
 *
 * Usage:
 *   node scripts/scrape-all-years-mtg.js
 *   node scripts/scrape-all-years-mtg.js --from=2010 --to=2000
 *   node scripts/scrape-all-years-mtg.js --dry-run
 */
...
const TO      = args.to    ? parseInt(args.to)    : 1993
...
// MTG : le "brand" correspond à l'éditeur (Wizards of the Coast)
const MAJOR_BRANDS = /(Wizards of the Coast|Wizards|WotC|Hasbro)/i

// Noms de sets MTG courants pour identifier les slugs TCDB
// TCDB utilise "MagicTheGathering" ou "Magic" comme sport slug
const MTG_SPORT_SLUGS = ['MagicTheGathering', 'Magic']
...
function loadCheckpoint() {
  ...
  catch { return { doneYears: [], doneTcdbIds: [], confirmedSlug: null } }
}
...
// Détecte automatiquement le bon slug TCDB pour MTG
async function detectMtgSlug(page, cp) {
  if (cp.confirmedSlug) return cp.confirmedSlug
  for (const slug of MTG_SPORT_SLUGS) {
    try {
      await waitCF(page, `${TCDB}/ViewAll.cfm/sp/${slug}/year/2024`)
      const sets = await page.evaluate(() => document.querySelectorAll('a[href*="sid/"]').length)
      if (sets > 0) {
        cp.confirmedSlug = slug
        saveCheckpoint(cp)
        console.log(`✓ Slug MTG confirmé: "${slug}"`)
        return slug
      }
    } catch {}
  }
  console.log('⚠️  Aucun slug MTG trouvé — utilisation de "MagicTheGathering" par défaut')
  return 'MagicTheGathering'
}

async function fetchSets(page, year, slug) {
  await waitCF(page, `${TCDB}/ViewAll.cfm/sp/${slug}/year/${year}`)
  await sleep(rand(500, 1000))
  return await page.evaluate(() => {
    const results = []
    const seen = new Set()
    // Pour MTG, on prend tous les sets listés (pas seulement Major Releases)
    document.querySelectorAll('a[href*="sid/"]').forEach(a => {
...
// MTG : scrape toutes les cartes directement depuis la page du set
    ...
    while (walker.nextNode()) {
      ...
      if (tag === 'TR') {
        ...
        for (const td of tds) {
          ...
          // Numéro de carte MTG : peut être chiffre, lettre, ou combinaison (ex: 001, M001, A)
          const isCardCode = /^\d+[a-zA-Z]?$/.test(rawText) || /^[A-Z]{0,3}\d+[a-zA-Z]?$/.test(rawText) || /^[A-Z]+$/.test(rawText)
          if (!cardNum && isCardCode && rawText.length <= 10) { cardNum = rawText; continue }
        ...
        cards.push({
          card_number: cardNum,
          player_name: cardName,  // "player_name" = nom de la carte MTG
          team: null,
          variation: currentVariation || null,
          is_rc: false,
          is_auto: /\bAU\b|\bAutograph\b|\bSigned\b/i.test(rowText),
        })
      }
    }
    return cards
...
async function scrapeSet(page, set, cp) {
  ...
  return { set, unique, brand: brandMatch ? brandMatch[1] : 'Wizards of the Coast' }
}

async function main() {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR)
  const cp = loadCheckpoint()
  console.log(`🧙 Scraper MTG — années ${FROM} → ${TO}`)
  ...
  const slug = await detectMtgSlug(page, cp)
  try {
    for (let yi = 0; yi < remaining.length; yi++) {
      const year = remaining[yi]
      console.log(`\n${'═'.repeat(60)}\n📅 Année MTG ${year} (${yi+1}/${remaining.length})\n${'═'.repeat(60)}`)
      if (yi > 0 && yi % 8 === 0) { page = await openBrowser() }
      let sets = []
      try { sets = await fetchSets(page, year, slug); console.log(`   ${sets.length} sets trouvés`) }
        ...
        try {
          ...
          if (result) {
            ...
            if (!DRY_RUN) {
              ...
              fs.writeFileSync(jsonFile, JSON.stringify({ year, sport: 'mtg', sets: [result] }, null, 2))

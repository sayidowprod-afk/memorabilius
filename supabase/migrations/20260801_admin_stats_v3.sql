-- admin_stats v3 : funnel activation, churn, rétention D30, DAU, qualité IA,
--                  complétude cartes, donateurs, top marques
...
WITH
  ...
  retention_d30 AS (
    ...
    WHERE EXISTS (
      SELECT 1 FROM cartes_manuelles c
      WHERE c.user_id = p.id
        AND c.created_at <= p.created_at + interval '30 days'
    )
    AND p.created_at <= now() - interval '30 days'
  ),
  -- Churn : actifs entre J-60 et J-30, disparus depuis J-30
  churn AS (
    SELECT
  ...
  -- Top champs corrigés par les utilisateurs
  top_corrections AS (

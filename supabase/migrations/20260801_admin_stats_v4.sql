-- admin_stats v4 : délai activation, comparaisons période précédente, prev churn/MAU
...
WITH
  ...
  -- Rétention D7 cohorte précédente (inscrits J-60 à J-30)
  prev_retention AS (
    SELECT count(DISTINCT p.id)::int AS retained
    FROM profiles p
    WHERE EXISTS (
  ...
  -- Churn période précédente (J-90 à J-60, disparus avant J-60)
  prev_churn AS (
    SELECT
      count(DISTINCT c.user_id)::int AS total_prev,
      count(DISTINCT c.user_id) FILTER (
  ...
  -- Délai médian activation (inscription → 1ère carte)
  activation AS (
...
-- jsonb_build_object est limité à 100 args (50 paires) — on splitte en 2 blocs reliés par ||
SELECT
  jsonb_build_object(
    -- ── Utilisateurs & cartes (15 paires) ─────────────────────────────────

...
DECLARE
  v_total int := 0; v_rc int := 0; v_auto int := 0;
  v_patch int := 0; v_num int := 0;
  v_mois  int := 0; v_views bigint := 0;
  v_teams int := 0;
  v_new   text[] := ARRAY[]::text[];
BEGIN
  SELECT COALESCE(stats_total,0), COALESCE(stats_rc,0), COALESCE(stats_auto,0),
         COALESCE(stats_patch,0), COALESCE(stats_num,0)
  INTO v_total, v_rc, v_auto, v_patch, v_num
  ...
  IF v_rc >= 1500 THEN v_new := v_new || 'rc_1500'; END IF;
  -- Autos
  IF v_auto >= 25   THEN v_new := v_new || 'auto_25';   END IF;
  IF v_auto >= 50   THEN v_new := v_new || 'auto_50';   END IF;
  IF v_auto >= 75   THEN v_new := v_new || 'auto_75';   END IF;
  IF v_auto >= 100  THEN v_new := v_new || 'auto_100';  END IF;
  IF v_auto >= 250  THEN v_new := v_new || 'auto_250';  END IF;
  IF v_auto >= 500  THEN v_new := v_new || 'auto_500';  END IF;
  IF v_auto >= 750  THEN v_new := v_new || 'auto_750';  END IF;
  IF v_auto >= 1000 THEN v_new := v_new || 'auto_1000'; END IF;
  IF v_auto >= 1500 THEN v_new := v_new || 'auto_1500'; END IF;
  -- Patch
  ...
  IF v_patch >= 1500 THEN v_new := v_new || 'patch_1500'; END IF;
  -- Numérotées
  ...
  IF v_num >= 1500 THEN v_new := v_new || 'num_1500'; END IF;

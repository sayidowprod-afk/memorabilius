...
export default function CardVideoExport({ card, accent: accentProp, onClose, ownerId }: Props) {
  ...
  const drawFrame = (ctx: CanvasRenderingContext2D, frontImg: HTMLImageElement, backImg: HTMLImageElement, p: number, holdT = 0) => {
    ...
    else if (p < 0.70) rot = Math.PI
    else if (p < 0.84) rot = Math.PI + Math.PI * easeInOut(seg(p, 0.70, 0.84))
    else               rot = 0
    ...
    const endFadeOut = holdT > 0 ? easeInOut(Math.min(1, Math.max(0, (holdT - 0.45) / 0.4))) : 0
    const endFadeIn  = holdT > 0 ? easeInOut(Math.min(1, Math.max(0, (holdT - 0.55) / 0.35))) : 0
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20, textAlign: 'left' }}>
            ...
            {teamThemes.length > 0 && (
              <div>
                ...
                {/* Menu deroulant groupe par sport plutot qu'une rangee de pastilles :
                    reste ergonomique meme avec beaucoup d'equipes favorites, et le
                    picker natif (mobile inclus) sait deja gerer une longue liste. */}
                <select
                  value={teamTheme?.key ?? ''}
                  onChange={e => {

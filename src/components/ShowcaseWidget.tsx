...
export default function ShowcaseWidget({ userId }: { userId: string }) {
  const [copied, setCopied] = useState<'html' | 'bbcode' | null>(null)
  ...
  const copy = (text: string, which: 'html' | 'bbcode') => {
  ...
  return (
    <div style={{ background: 'white', borderRadius: 16, padding: 30, boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: 20 }}>
      ...
      <p style={{ fontSize: 13, color: '#666', marginBottom: 16 }}>
        Affichez votre collection sur un forum, un blog ou en signature — la bannière se met à jour automatiquement avec vos dernières cartes.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div>
          <label style={{ fontSize: 11, fontWeight: 800, color: '#888', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Code HTML</label>

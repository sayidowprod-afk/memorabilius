...
export default function TeamPicker({ value, onChange, max = 5 }: Props) {
  ...
  const sports: Sport[] = ['nba', 'wnba', 'nfl', 'mlb', 'nhl', 'football', 'ncaa']
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card-bg, #fff)', color: 'var(--text, #121212)', borderRadius: 16, padding: 20, width: '100%', maxWidth: 520, maxHeight: '80vh', display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => { setOpen(false); setSearch('') }} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--text3, #999)' }}>✕</button>
            </div>

            {/* Sport tabs */}
            <div style={{ display: 'flex', gap: 4, background: 'var(--bg3, #f5f5f5)', borderRadius: 10, padding: 4 }}>
              {sports.map(s => (
                <button key={s} onClick={() => { setSport(s); setSearch('') }} style={{
                  ...
                  background: sport === s ? 'var(--card-bg, #fff)' : 'transparent',
                  color: sport === s ? 'var(--text, #121212)' : 'var(--text2, #888)',
            ...
            <input
              ...
              style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border, #ddd)', fontSize: 13, background: 'var(--card-bg, #fff)', color: 'var(--text, #121212)' }}
              autoFocus
            />
                ...
                {filtered.map(team => {
                  ...
                  return (
                    <button key={team.id} onClick={() => toggle(team.id)} disabled={disabled} style={{
                      ...
                      <span style={{ fontSize: 9, fontWeight: 800, color: selected ? team.color : 'var(--text2, #666)', textAlign: 'center', lineHeight: 1.2 }}>{team.name.length > 12 ? team.abbr : team.name}</span>
                    </button>
                  )
                })}
              </div>
            ...
            <p style={{ fontSize: 11, color: 'var(--text3, #bbb)', margin: 0, textAlign: 'center' }}>

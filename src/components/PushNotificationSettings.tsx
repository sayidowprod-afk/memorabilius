...
export default function PushNotificationSettings({ dark }: { dark: boolean }) {
  ...
  // runGatedNativeCall() protege contre un crash natif Capacitor confirme sur
  // appareil reel (voir src/lib/nativePermissionGate.ts).
  useEffect(() => {
    if (!isNative) return
    let cancelled = false
    import('@capacitor/push-notifications').then(async ({ PushNotifications }) => {
      const p = await runGatedNativeCall(() => PushNotifications.checkPermissions())
      if (!cancelled) setNativePushPermission(p.receive as any)
    }).catch(() => {})

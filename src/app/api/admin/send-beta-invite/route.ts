import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'
import { Resend } from 'resend'

const admin = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
)

const ADMIN_EMAILS = new Set([
  'kikibajkiki@gmail.com',
  'killian.bajoni@hotmail.fr',
  'killianbajoni@hotmail.com',
  'sayidowprod@gmail.com',
  ...(process.env.ADMIN_EMAIL ? [process.env.ADMIN_EMAIL] : []),
].map(e => e.toLowerCase()))

function getResend() { return new Resend(process.env.RESEND_API_KEY!) }

// Default sheet: liste testeurs bêta
const DEFAULT_SHEET_ID = '133jN61dV9QIRam7bLQWH4zOo21JT-G4rXEXuRYJO_qA'

const LOGO_B64 = ''

function sheetIdFromUrl(url: string): string | null {
  const m = url.match(/spreadsheets\/d\/([\w-]+)/)
  return m ? m[1] : null
}

async function fetchEmailsFromSheet(sheetUrl: string): Promise<string[]> {
  const id = sheetIdFromUrl(sheetUrl) ?? sheetUrl
  const csvUrl = id.startsWith('http')
    ? id
    : `https://docs.google.com/spreadsheets/d/${id}/export?format=csv`

  const res = await fetch(csvUrl)
  if (!res.ok) throw new Error(`Impossible de lire le Google Sheet (${res.status})`)
  const text = await res.text()

  const emails: string[] = []
  for (const line of text.split('\n')) {
    const cell = line.split(',')[0].trim().replace(/^"|"$/g, '')
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cell)) {
      emails.push(cell.toLowerCase())
    }
  }
  return [...new Set(emails)]
}

function esc(s: string) {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
}

function buildBetaEmail(email: string): string {
  const optInUrl = 'https://play.google.com/apps/testing/fr.memorabilius.app'
  return `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Tu es invité à tester Memorabilius sur Android !</title>
</head>
<body style="margin:0;padding:0;background:#e8eaed;font-family:-apple-system,'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:640px;margin:30px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.13);">
    <div style="background:#003DA6;padding:24px 40px;text-align:center;">
      <img src="data:image/png;base64,${LOGO_B64}" alt="Memorabilius" style="height:44px;display:inline-block;">
    </div>
    <div style="background:linear-gradient(135deg,#0a4fd4 0%,#003DA6 100%);padding:22px 40px;display:flex;align-items:center;gap:16px;">
      <div style="font-size:32px;line-height:1;flex-shrink:0;">🎉</div>
      <div>
        <p style="margin:0;font-size:18px;font-weight:800;color:#ffffff;line-height:1.2;">Tu es invité à tester l'application Android&nbsp;!</p>
        <p style="margin:4px 0 0;font-size:13px;color:rgba(255,255,255,0.75);font-weight:500;">Accès bêta exclusif — Memorabilius v1.0</p>
      </div>
    </div>
    <div style="padding:36px 40px 12px;">
      <p style="font-size:15px;color:#202124;margin:0 0 16px;line-height:1.6;">Salut,</p>
      <p style="font-size:15px;color:#3c3c3c;line-height:1.75;margin:0 0 20px;">
        On a sélectionné un petit groupe de testeurs de confiance pour essayer
        <strong style="color:#003DA6;">Memorabilius sur Android</strong> avant le lancement officiel sur le Play Store.<br>
        Tu fais partie de l'aventure&nbsp;!
      </p>
      <div style="background:#f8f9ff;border-radius:12px;padding:22px 24px;margin:0 0 24px;">
        <p style="font-size:12px;font-weight:700;color:#003DA6;text-transform:uppercase;letter-spacing:.08em;margin:0 0 16px;">Comment rejoindre le test</p>
        <div style="display:flex;align-items:flex-start;gap:14px;margin:0 0 14px;">
          <div style="flex-shrink:0;width:28px;height:28px;background:#003DA6;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:800;line-height:28px;text-align:center;">1</div>
          <div>
            <p style="margin:0;font-size:14px;font-weight:700;color:#202124;">Ouvre le lien depuis ton téléphone Android</p>
            <p style="margin:3px 0 0;font-size:13px;color:#5f6368;line-height:1.5;">Le bouton ci-dessous doit être ouvert depuis un appareil Android avec le compte Google associé à <strong>${esc(email)}</strong>.</p>
          </div>
        </div>
        <div style="display:flex;align-items:flex-start;gap:14px;margin:0 0 14px;">
          <div style="flex-shrink:0;width:28px;height:28px;background:#003DA6;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:800;line-height:28px;text-align:center;">2</div>
          <div>
            <p style="margin:0;font-size:14px;font-weight:700;color:#202124;">Clique sur "Rejoindre le programme"</p>
            <p style="margin:3px 0 0;font-size:13px;color:#5f6368;line-height:1.5;">Google te demande d'accepter de rejoindre la version bêta.</p>
          </div>
        </div>
        <div style="display:flex;align-items:flex-start;gap:14px;">
          <div style="flex-shrink:0;width:28px;height:28px;background:#00B07D;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:800;line-height:28px;text-align:center;">3</div>
          <div>
            <p style="margin:0;font-size:14px;font-weight:700;color:#202124;">Télécharge l'app et explore !</p>
            <p style="margin:3px 0 0;font-size:13px;color:#5f6368;line-height:1.5;">Gère ta collection, scanne tes cartes, partage ta galerie.</p>
          </div>
        </div>
      </div>
      <div style="text-align:center;margin:28px 0;">
        <a href="${optInUrl}" style="display:inline-block;background:#00B07D;color:#fff;text-decoration:none;font-size:16px;font-weight:700;padding:16px 40px;border-radius:10px;letter-spacing:.01em;">
          📲 Rejoindre le test bêta
        </a>
        <p style="margin:10px 0 0;font-size:12px;color:#9aa0a6;">⚠️ Ouvre ce lien depuis ton téléphone Android</p>
      </div>
      <div style="background:#fffbeb;border-left:3px solid #f59e0b;border-radius:0 8px 8px 0;padding:12px 16px;margin:0 0 28px;">
        <p style="margin:0;font-size:13px;color:#92400e;line-height:1.5;">
          <strong>Important&nbsp;:</strong> si tu n'as pas accès à l'app dans les 24h après avoir cliqué, réponds à ce mail en indiquant l'adresse Google de ton téléphone Android.
        </p>
      </div>
      <div style="padding-top:20px;border-top:1px solid #e8e8e8;">
        <p style="font-size:14px;color:#5f6368;margin:0 0 4px;">Merci pour ton soutien,</p>
        <p style="font-size:14px;font-weight:700;color:#003DA6;margin:0;">L'équipe Memorabilius</p>
      </div>
    </div>
    <div style="background:#f8f8f8;padding:16px 40px;border-top:1px solid #e8e8e8;text-align:center;margin-top:20px;">
      <p style="font-size:11px;color:#9aa0a6;margin:0;">
        Tu reçois cet e-mail car tu as été sélectionné comme testeur bêta Memorabilius.&nbsp;
        <a href="https://www.memorabilius.fr" style="color:#9aa0a6;">memorabilius.fr</a>
      </p>
    </div>
  </div>
</body>
</html>`
}

export async function POST(req: NextRequest) {
  try {
    const token = req.headers.get('authorization')?.replace('Bearer ', '')
    if (!token) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })

    const { data: { user } } = await admin.auth.getUser(token)
    if (!user || !ADMIN_EMAILS.has(user.email?.toLowerCase() ?? '')) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }

    const body = await req.json().catch(() => ({}))
    const sheetsUrl: string = body.sheets_url || `https://docs.google.com/spreadsheets/d/${DEFAULT_SHEET_ID}/export?format=csv`

    let emails: string[]
    try {
      emails = await fetchEmailsFromSheet(sheetsUrl)
    } catch (e) {
      return NextResponse.json({ error: String(e) }, { status: 400 })
    }

    if (emails.length === 0) {
      return NextResponse.json({ sent: 0, total: 0, message: 'Aucune adresse email valide trouvée dans le sheet.' })
    }

    const resend = getResend()
    let sent = 0
    const errors: string[] = []

    for (const email of emails) {
      try {
        await resend.emails.send({
          from: 'Memorabilius <contact@memorabilius.fr>',
          to: email,
          subject: '📲 Tu es invité à tester Memorabilius sur Android !',
          html: buildBetaEmail(email),
        })
        sent++
      } catch {
        errors.push(email)
      }
    }

    return NextResponse.json({ sent, total: emails.length, errors })
  } catch (err) {
    return NextResponse.json({ error: String(err) }, { status: 500 })
  }
}

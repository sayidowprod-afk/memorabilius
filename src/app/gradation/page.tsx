...
export default function EtatCartePage() {
  ...
  const loadFromGallery = async (card: PickableCard) => {
    ...
    setError('')
    try {
      // Passe par le proxy same-origin -- un fetch direct vers une image
      // externe (CSV) ou le storage Supabase echoue en pratique (CORS), voir
      // src/app/api/proxy-image (qui valide aussi la securite cote serveur :
      // IP privee/interne refusee, type de contenu, taille).

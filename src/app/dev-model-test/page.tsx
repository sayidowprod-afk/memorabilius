...
import { refineCornersV4, refineCornersV5 } from '@/lib/cornerDetectorYolo'
...
// prod sur le scanner aujourd'hui), "test actuel" (v4, juge meilleur que v3
// lors du dernier essai -- bord physique partage + seuil relatif) et
// "nouvelle version test" (v5 : v4 + polissage final a tres petite echelle
// + garde-fou ratio d'aspect carte standard). Aucune des versions de
...
const VARIANTS = [
  ...
  { key: 'v4', label: 'Test actuel (v4 -- bord partage + seuil relatif)', color: '#e74c3c' },
  { key: 'v5', label: 'Nouvelle version test (v5 -- polissage final + ratio d\'aspect)', color: '#ff8c00' },
] as const
...
export default function DevModelTest() {
  ...
  const onFile = async (file: File) => {
    ...
    try {
      ...
      const v4Ms = detectMs + (performance.now() - t1)

      const t2 = performance.now()
      const v5Corners = rawCorners ? refineCornersV5(img, rawCorners, scale) : null
      const v5Ms = detectMs + (performance.now() - t2)

      const variants = { actuel: rawCorners, v4: v4Corners, v5: v5Corners }
      setCorners(variants)
      setInfo({
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 14px 60px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <p style={{ fontSize: 13, color: '#888', marginBottom: 4 }}>
        3 sections : brut (ce qui tourne en prod aujourd'hui), v4 (test actuel), et nouvelle version v5
      </p>

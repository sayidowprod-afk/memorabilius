...
export default function Trades() {
  ...
  const [offerTab, setOfferTab] = useState<'pending' | 'history'>('pending')
  ...
  useEffect(() => {
    if (mainTab === 'echanges' && tradeOffers.length === 0 && !loadingOffers) loadTradeOffers()
  }, [mainTab])
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* ══════════════════════════════════════════════════════════════════════ */}
      {/* Onglet MES ÉCHANGES (offres directes)                                 */}
      {/* ══════════════════════════════════════════════════════════════════════ */}
      {mainTab === 'echanges' && (
                  ...
                  <div key={trade.id} style={{ background: 'var(--card-bg, #fff)', borderRadius: 16, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.07)', border: '1.5px solid var(--border, #f0f0f0)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {trade.status === 'pending' && !isSender && (
                        <>
                          <button disabled={acting !== null} onClick={() => actOnOffer(trade.id, 'accept')}
            ...
            <div className="trade-popup-inner" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', flex: 1, minHeight: 0 }}>
              <div className="trade-popup-img" style={{ background: 'var(--bg3, #f8f8f8)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                ...
                {popup._source === 'galerie' && popup.image_verso && (
                  // Bas-gauche : les 3 autres coins sont pris (badge type en
                  // haut-gauche, fermer en haut-droite, hint Zoom en bas-droite).
                  <button onClick={flipPopupVerso} style={{ position: 'absolute', bottom: 12, left: 12, background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', borderRadius: 20, padding: '6px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                ...
                {/* Croix de fermeture flottante sur l'image plutot que premier
                    ...
                    conventionnelle pour un bouton fermer de modale (coin
                    haut-droite), et evite qu'elle pousse le titre vers le bas. */}
                <button onClick={() => setPopup(null)} aria-label="Fermer" style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', width: 32, height: 32, borderRadius: '50%', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 'auto' }}>
                  ...
                  {userId === popup.user_id && !popup._source && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      <button onClick={() => closeTrade(popup.id)} style={{ background: '#fff5f5', color: '#e67e22', padding: '12px', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                      ...
                      <button onClick={async () => {
                        ...
                        setPopup(null)
                      }} style={{ background: '#fff5f5', color: '#e74c3c', padding: '12px', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>

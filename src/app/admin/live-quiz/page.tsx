...
interface Question {
  id: string; question: string; choices: string[]; correct_index: number; used: boolean; duration_seconds: number | null
}
...
export default function LiveQuizAdminPage() {
  ...
  const addQuestion = async () => {
    ...
    const tok = await freshToken()
    if (!tok) return
    const durationSeconds = qDuration.trim() ? Number(qDuration) : undefined
    const res = await fetch('/api/admin/live-quiz/questions', {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${tok}` },
      body: JSON.stringify({ sessionId: activeId, question: qText.trim(), choices, correctIndex: Math.min(qCorrect, choices.length - 1), durationSeconds }),
    })
    if (!res.ok) { alert('Erreur ajout question'); return }
    setQText(''); setQChoices(['', '', '', '']); setQCorrect(0); setQDuration('')
    loadActive(activeId)
  }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 20, minWidth: 0 }} className="live-quiz-panels">
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 18 }}>
                {questions.map(q => (
                  <div key={q.id} style={{ padding: 10, borderRadius: 10, border: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, opacity: q.used ? 0.5 : 1, flexWrap: 'wrap' }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{q.question}{q.duration_seconds ? ` · ⏱ ${q.duration_seconds}s` : ''}</div>
                      <div style={{ fontSize: 11, color: '#888' }}>{q.choices.join(' · ')}</div>
                    </div>
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      <button onClick={() => runAction('start_round', q.id)} disabled={session.status === 'question'} style={btnStyle(FDLC_NAVY)}>Lancer</button>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <label style={{ fontSize: 12, fontWeight: 700, color: '#888', flexShrink: 0 }}>⏱ Minuteur (s, optionnel)</label>
                <input type="number" min={5} value={qDuration} onChange={e => setQDuration(e.target.value)} placeholder="ex: 20"
                  style={{ width: 90, minWidth: 0, padding: 8, borderRadius: 8, border: '1px solid #ddd', boxSizing: 'border-box' }} />
              </div>
              <button onClick={addQuestion} style={btnStyle(FDLC_RED)}>+ Ajouter</button>
            </div>
          </div>
        )}
      </div>

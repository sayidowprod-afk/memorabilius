...
import { runGatedNativeCall } from './nativePermissionGate'
...
async function ensurePermission(): Promise<boolean> {
  ...
  try {
    ...
    const perm = await runGatedNativeCall(() => LocalNotifications.checkPermissions())
    if (perm.display === 'granted') return true
    const req = await runGatedNativeCall(() => LocalNotifications.requestPermissions())
